...
export class MyRemoteService {
    ...
    getCelsius(fahrenheitInput: string): Observable<string[]> {
        let content = new URLSearchParams();
        content.set('f', fahrenheitInput);
    ...
    getFahrenheit(celsiusInput: string): Observable<string[]> {
        let content = new URLSearchParams();
        content.set('c', celsiusInput);
        let headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
        let options = new RequestOptions({
            search: content
        });
